...
import { startTunnel, namedTunnelConfig } from './tunnel.js';
...
let named;
try { named = tunnelMode ? namedTunnelConfig(process.env.CLOUDFLARE_TUNNEL_TOKEN, process.env.PUBLIC_URL) : null; }
...
if (named) origins.push(named.publicUrl);
...
const listenerUrls = named ? [named.publicUrl] : local ? [origins[0]] : origins.slice(2);
const sessionTokens = loadSession(`${root}.local`);
...
const office = createOfficeServer({ serverFactory: app => local ? http.createServer(app) : https.createServer(tls, app), dist: `${root}dist`, origins, listenerUrls, tunnel: tunnelMode, rtcProvider: rtc.get, turnEnabled: rtc.enabled, sessionTokens });
office.server.listen(port, local ? '127.0.0.1' : '0.0.0.0', async () => {
  if (tunnelMode) {
    console.log('Avataan Cloudflare HTTPS -tunnelia…');
    tunnel = startTunnel(port, { ...(named || {}), onExit: () => { console.error('Cloudflare-tunneli sulkeutui. Käynnistä npm run tunnel uudelleen.'); shutdown(1); } });
    try {
